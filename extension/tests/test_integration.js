...
async function saveSummary() {
  if (!currentState.summary || !currentState.videoData) return false;
...
describe('Extension Integration', () => {
  ...
  describe('Complete summarization flow', () => {
    test('should complete the full summarization process', async () => {
      ...
      // 3. Check for video
      await checkForVideo();
      expect(currentState.videoData).not.toBeNull();
      ...
      expect(currentState.summary).toBe('This is the completed summary of the integration test video.');
